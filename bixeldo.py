...
def gettemphum():
    ht = Adafruit_DHT.read_retry(22,4)
    global temp
    temp = round(ht[1], 1)
    global hum
    hum = round(ht[0], 1)
...
def plotgraph():
    ...
    #make set temp line:
    settemp = np.empty((2,2))
    settemp[0,1] = int(float(dicts['SetTemp']))
    settemp[1,1] = int(float(dicts['SetTemp']))
    settemp[0,0] = Atemp[0,0]
    settemp[1,0] = Atemp[-1,0]
    #
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    ax1.plot(Atemp[:,0], Atemp[:,1], label = 'Temperature')
    ax1.plot(Ahum[:,0], Ahum[:,1], label = 'Humidity')
    ax1.plot(settemp[:,0], settemp[:,1], label = 'Set Temperature', linestyle = ':')
...
#    print('if protocol is live flag yes')
    ...
    if temp > float(dicts['SetTemp']):
        fanon()
        print("fan on. temp = "+ str(temp)+ ", set temp = "+ str(dicts['SetTemp']))
    else:
        fanoff()
        print("fan off. temp = "+ str(temp)+ ", set temp = "+ str(dicts['SetTemp']))
    #Get minutes elapsed since experiment start:
##
    fmt = '%Y/%m/%d-%H:%M:%S'
    d1 = datetime.strptime(dicts['DateStart'] + '-' + dicts['TimeStart'], fmt)
    d2 = datetime.strptime(str(date) + '-' + str(timenow), fmt)

    # Convert to Unix timestamp
    d1_ts = time.mktime(d1.timetuple())
    d2_ts = time.mktime(d2.timetuple())
    mins = math.floor(int(d2_ts-d1_ts) / 60)
##
